...
Valid_ROC <- function(obs, pred, direction = 0, threshold  = NULL, graph = T)
{
  require(pROC)
  require(caret)
  if(!is.factor(obs)){obs <- factor(obs)} # si obs n'est pas un facteur alors obs est convertit avec factor
  lev        <- levels(obs)
  data_logit <- data.frame(obs = factor(obs), pred = pred) # création d'un dataframe qui contient 2 colonnes obs et pred les obs (observations réelles) et les preds(prédictions du modèle)
  data_roc   <- roc(obs,pred)  # On calcul la courbes roc avec obs et pred
  cir        <- ci(data_roc)  # ci Calcul les intervalles de confiance de l'AUC de la courbe roc
  
  # data_spec  contient les sensibilités, spécificités, seuils et l'indice Youden(sensibilité + spécificité - 1)
...
* Le MDS, est une autre façon de représenter clairement l’information contenue dans une matrice de distances.
  + On utilise MDSplot (Multi-dimensional Scaling Plot of Proximity matrix from randomForest) pour tracer les coordonnées de mise à l'échelle de la forêt aléatoire rf1 dont nous disposons.

```{r}
MDSplot(rf1, rf1$predicted, k=2 ,pch=as.numeric(rf1$predicted))
```
Dans la figure précédente, On observe une forme en U inversée. Les couleurs (bleu- rouge) des points correspondent au classement effectué par l’algorithme. La position des individus (et de leurs distance entre eux) est obtenue à partir d’une diminution de dimension par analyse factorielle (analogue au projection ACP) Comme on peut le constater, la méthode des forêts aléatoires permet une bonne classification des individus même dans le cas des zones à risque (partie supérieure du graphique: forte proximité des points) ou des individus dans les deux branches.


##### <FONT color='#99CCFF'> 4.2 Importance des variables </FONT>

Distribution de la profondeur des noeuds en fonction des variables

* Une fonctionnalité très intéressante est la mesure de l’importance de chaque variable. Nous avons vu en cours le principe général permettant d’estimer l’importance des variables par permutation aléatoire. Le graphique suivant fournit la distribution des profondeurs minimales par variable. Plus la profondeur est faible plus la variable est ‘importante’. Ce graphique utilise le critère mean decrease Gini

On veut afficher un diagramme en barres à barres cumulées : 
...
Importance des variables Mean Decrease Gini ou Mean Decrease Accuracy

* Il vaut mieux utiliser le critère Mean Decrease Accuracy car il est plus général. En effet, le critère “mean decrease Gini”” ne s’applique qu’à la construction des arbres CART. Le graphe suivant fournit l’importance des variables (en % da la totalité des importances de toutes les variables)
...
importance_df <- data.frame(
  Variable = row.names(rf1$importance),
  MeanDecreaseAccuracy = rf1$importance[,3],
  stringsAsFactors = FALSE
)

# Calcul de la somme totale de toutes les importances
total_importance <- sum(importance_df$MeanDecreaseAccuracy)
# Calcul des pourcentages de chaque variable
importance_df$Percentage <- importance_df$MeanDecreaseAccuracy / total_importance * 100
# Création du graphique en barres
ggplot(importance_df, aes(x = reorder(Variable, -Percentage), y = Percentage, fill = Variable)) +
  geom_bar(stat = "identity") +
  labs(x = "Variable", y = "Importance (%) Mean Decrease Accuracy") +
...
importance_df2 <- data.frame(
  Variable = row.names(rf1$importance),
  MeanDecreaseGini = rf1$importance[,4],
  stringsAsFactors = FALSE
)

# Calcul de la somme totale de toutes les importances
total_importance2 <- sum(importance_df2$MeanDecreaseGini)

# Calcul des pourcentages de chaque variable
importance_df2$Percentage <- importance_df2$MeanDecreaseGini / total_importance /10
# Création du graphique en barres
ggplot(importance_df2, aes(x = reorder(Variable, -Percentage), y = Percentage, fill = Variable)) +
  geom_bar(stat = "identity") +
  labs(x = "Variable", y = "Importance (%) Mean Decrease Gini") +
  theme(plot.title = element_text(hjust = 0.5), 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_fill_brewer(palette = "Paired")
...
importance_df3 <- data.frame(
  Variable = row.names(rf2$importance),
  MeanDecreaseAccuracy = rf2$importance[,3],
  stringsAsFactors = FALSE
)

# Calcul de la somme totale de toutes les importances
total_importance3 <- sum(importance_df3$MeanDecreaseAccuracy)
# Calcul des pourcentages de chaque variable
importance_df3$Percentage <- importance_df3$MeanDecreaseAccuracy / total_importance3 * 100
# Création du graphique en barres
ggplot(importance_df3, aes(x = reorder(Variable, -Percentage), y = Percentage, fill = Variable)) +
  geom_bar(stat = "identity") +
  labs(x = "Variable", y = "Importance (%) Mean Decrease Accuracy") +
  theme(plot.title = element_text(hjust = 0.5), 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
# Création d'un data.frame avec les données
importance_df4 <- data.frame(
  Variable = row.names(rf2$importance),
  MeanDecreaseGini = rf2$importance[,4],
  stringsAsFactors = FALSE
)

# Calcul de la somme totale de toutes les importances
total_importance4 <- sum(importance_df4$MeanDecreaseGini)

# Calcul des pourcentages de chaque variable
importance_df4$Percentage <- importance_df4$MeanDecreaseGini / total_importance4 *100
# Création du graphique en barres
ggplot(importance_df4, aes(x = reorder(Variable, -Percentage), y = Percentage, fill = Variable)) +
  geom_bar(stat = "identity") +
  labs(x = "Variable", y = "Importance (%) Mean Decrease Gini") +
  theme(plot.title = element_text(hjust = 0.5), 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
